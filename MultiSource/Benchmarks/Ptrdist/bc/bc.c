...
short yydef _Checked[]={
		...
		0,  12,  31,  49,   0,  32 };
#ifndef lint
static _TPtr<char> __ConstantStringToTainted__(char* ipStr, long ipLen) {
	_TPtr<char> ret = t_malloc(ipLen+1);
	memcpy(ret, ipStr, ipLen);
	ret[ipLen] = '\0';
	return ret;
}


//_TPtr<char> yaccpar_sccsid = __ConstantStringToTainted__("@(#)yaccpar	4.1	(Berkeley)	2/11/83", strlen("@(#)yaccpar	4.1	(Berkeley)	2/11/83"));
#endif

static _TLIB _TPtr<void> __ConstantArrayToTainted__(char* ipPtr, long IpSize) {
	_TPtr<void> ret = t_malloc(IpSize);
	t_memcpy(ret, ipPtr, IpSize);
	return ret;
}
#
# define YYFLAG -1000
...
yyparse() {
	...
	*yypv = yyval;
	...
	if( (yyn=yydef[yystate]) == -2 ) {
		...
		for( yyxi=(short*)yyexca; (*yyxi!= (-1)) || (yyxi[1]!=yystate) ; yyxi += 2 ) ; /* VOID */
	...
	switch(yym){
		...
		case 31:
		{
			...
			if (yypvt[-1].i_value < 0)
				sprintf (((char *)genstr), "J%1d:N%1d:", yypvt[-7].i_value, yypvt[-4].i_value);
			else
				sprintf (((char *)genstr), "pJ%1d:N%1d:", yypvt[-7].i_value, yypvt[-4].i_value);
			generate ((genstr));
		} break;
		case 32:
		{
			_Unchecked { sprintf (((char *)genstr), "J%1d:N%1d:",
		...
		case 33:
		{
			...
			if_label = next_label++;
			_Unchecked { sprintf (((char *)genstr), "Z%1d:", if_label); };
			generate ((genstr));
		} break;
		case 34:
		{
			_Unchecked { sprintf (((char *)genstr), "N%1d:", if_label); };
			generate ((genstr));
		...
		case 35:
		{
			...
			_Unchecked { sprintf (((char *)genstr), "N%1d:", yypvt[-0].i_value); };
			generate ((genstr));
		} break;
		case 36:
		{
			yypvt[-0].i_value = break_label;
			break_label = next_label++;
			_Unchecked { sprintf (((char *)genstr), "Z%1d:", break_label); };
			generate ((genstr));
		} break;
		case 37:
		{
			_Unchecked { sprintf (((char *)genstr), "J%1d:N%1d:", yypvt[-6].i_value, break_label); };
			generate ((genstr));
		...
		{ yyval.i_value = 0; } break;
		case 39:
			_Unchecked {  warn ("print statement"); } break;
		case 43:
		{
			generate (("O"));
			generate (yypvt[-0].s_value);
			t_free<char>(yypvt[-0].s_value);
		} break;
		case 44:
		{ generate (("P")); } break;
		case 46:
		{
			_Unchecked { warn ("else clause in if statement"); };
			yypvt[-0].i_value = next_label++;
			_Unchecked { sprintf (((char *)genstr), "J%d:N%1d:", yypvt[-0].i_value, if_label); };
			generate ((genstr));
		...
		case 48:
		{
			...
			sprintf (genstr, "F%d,%s.%s[", lookup(yypvt[-6].s_value,FUNCT),
					 ((char *)arg_str (yypvt[-4].a_value,TRUE)), ((char *)arg_str (yypvt[-0].a_value,TRUE)));
			generate ((genstr));
		...
		case 50:
			_Unchecked { yyval.a_value = NULL; } break;
		case 52:
			_Unchecked { yyval.a_value = NULL; } break;
		case 53:
		...
		{ yyval.a_value = yypvt[-1].a_value; } break;
		case 55:
			_Unchecked { yyval.a_value = nextarg (NULL, lookup (yypvt[-0].s_value,SIMPLE)); } break;
		case 56:
			_Unchecked { yyval.a_value = nextarg (NULL, lookup (yypvt[-2].s_value,ARRAY)); } break;
		case 57:
		...
		case 62:
			_Unchecked {
			sprintf (((char *)genstr), "K%d:", -lookup (yypvt[-2].s_value,ARRAY));
			generate ((genstr));
		...
		case 64:
		{
			_Unchecked { sprintf (((char *)genstr), "K%d:", -lookup (yypvt[-2].s_value,ARRAY)); };
			generate ((genstr));
		...
		{
			yyval.i_value = -1;
			_Unchecked { warn ("Missing expression in for statement"); };
		} break;
		case 67:
		{
			yyval.i_value = 0;
		...
		case 69:
		{
			if (yypvt[-0].c_value != '=')
				_Unchecked {
				if (yypvt[-1].i_value < 0)
					sprintf (((char *)genstr), "DL%d:", -yypvt[-1].i_value);
				else
					sprintf (((char *)genstr), "l%d:", yypvt[-1].i_value);
				generate ((genstr));
			}
		} break;
		case 70:
			...
			if (yypvt[-2].c_value != '=')
			{
				_Unchecked { sprintf (((char *)genstr), "%c", yypvt[-2].c_value); };
				generate ((genstr));
			}
			if (yypvt[-3].i_value < 0)
				sprintf (((char *)genstr), "S%d:", -yypvt[-3].i_value);
			else
				sprintf (((char *)genstr), "s%d:", yypvt[-3].i_value);
			generate ((genstr));
			yyval.i_value = 0;
		...
		case 71:
		{
			_Unchecked { warn("&& operator"); };
			yypvt[-0].i_value = next_label++;
			_Unchecked { sprintf (((char *)genstr), "DZ%d:p", yypvt[-0].i_value); };
			generate ((genstr));
		} break;
		case 72:
		{
			_Unchecked { sprintf (((char *)genstr), "DZ%d:p1N%d:", yypvt[-2].i_value, yypvt[-2].i_value); };
			generate ((genstr));
		...
		case 73:
		{
			_Unchecked { warn("|| operator"); };
			yypvt[-0].i_value = next_label++;
			_Unchecked { sprintf (((char *)genstr), "B%d:", yypvt[-0].i_value); };
			generate ((genstr));
		} break;
		case 74:
		{
			int tmplab;
			tmplab = next_label++;
			_Unchecked { sprintf (((char *)genstr), "B%d:0J%d:N%d:1N%d:",
		...
		case 82:
			...
			if (yypvt[-0].i_value < 0)
				sprintf (((char *)genstr), "L%d:", -yypvt[-0].i_value);
			else
				sprintf (((char *)genstr), "l%d:", yypvt[-0].i_value);
			generate ((genstr));
		} break;
		...
		case 85:
			...
			{
				_Unchecked { sprintf (((char *)genstr), "C%d,%s:",
			...
			{
				sprintf (((char *)genstr), "C%d:", lookup (yypvt[-3].s_value,FUNCT));
			}
			generate ((genstr));
		} break;
		case 86:
		{
			...
			if (yypvt[-0].i_value < 0)
				_Unchecked {
				if (yypvt[-1].c_value == '+')
					sprintf (((char *)genstr), "DA%d:L%d:", -yypvt[-0].i_value, -yypvt[-0].i_value);
				else
					sprintf (((char *)genstr), "DM%d:L%d:", -yypvt[-0].i_value, -yypvt[-0].i_value);
			}
			else
			_Unchecked {
				if (yypvt[-1].c_value == '+')
					sprintf (((char *)genstr), "i%d:l%d:", yypvt[-0].i_value, yypvt[-0].i_value);
				else
					sprintf (((char *)genstr), "d%d:l%d:", yypvt[-0].i_value, yypvt[-0].i_value);
			}
			generate ((genstr));
		} break;
		case 87:
		{
			...
			if (yypvt[-1].i_value < 0)
				_Unchecked {
				sprintf (((char *)genstr), "DL%d:x", -yypvt[-1].i_value);
				generate ((genstr));
				if (yypvt[-0].c_value == '+')
					sprintf (((char *)genstr), "A%d:", -yypvt[-1].i_value);
				else
					sprintf (((char *)genstr), "M%d:", -yypvt[-1].i_value);
			}
			else
			_Unchecked {
				sprintf (((char *)genstr), "l%d:", yypvt[-1].i_value);
				generate ((genstr));
				if (yypvt[-0].c_value == '+')
					sprintf (((char *)genstr), "i%d:", yypvt[-1].i_value);
				else
					sprintf (((char *)genstr), "d%d:", yypvt[-1].i_value);
			}
			generate ((genstr));
		} break;
		case 88:
