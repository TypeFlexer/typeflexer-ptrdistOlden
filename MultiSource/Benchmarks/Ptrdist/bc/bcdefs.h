...
typedef Tstruct arg_list
{
    int av_name;
    _TPtr<Tstruct arg_list> next;
} arg_list;
...
typedef struct
{
    ...
    _TPtr<Tstruct arg_list>  f_params;
    _TPtr<Tstruct arg_list>  f_autos;
} bc_function;
...
typedef Tstruct estack_rec {
        bc_num s_num;
        _TPtr<Tstruct estack_rec> s_next;
} estack_rec;

typedef Tstruct fstack_rec {
        int  s_val;
        _TPtr<Tstruct fstack_rec> s_next;
} fstack_rec;


/* The following are for the name tree. */

typedef Tstruct id_rec {
        _TPtr<char>  id;   /* The program name. */
        ...
        _TPtr<Tstruct id_rec>  left;
        _TPtr<Tstruct id_rec> right; /* Tree pointers. */
} id_rec;