...
execute (void)
{
	...
	bc_num temp_num;
	_TPtr<Tstruct arg_list> auto_list = NULL;
	;
	...
	int depth; _TPtr<estack_rec> temp = ex_stack;
	...
	else
	  {
		...
		switch ( inst )
		{
			...
			case 'P' : /* Write the value on the top of the stack.  No newline. */
				out_num (ex_stack->s_num, o_base, _Assume_bounds_cast<_Ptr<void (int)>>((void (*)(int))out_char));
				if (inst == 'W') out_char ('\n');
...
void
push_constant (_Ptr<char (void)> in_char, int conv_base)
{
	int digits;
...
void
push_b10_const (_Ptr<program_counter> pc)
{
	bc_num build;
