...
more_functions (void)
{
    ...
    f_count += STORE_INCR;
    functions = (bc_function *)malloc<bc_function>(f_count*sizeof (bc_function));
    f_names = (char * *)malloc<char *>(f_count*sizeof (char *));
...
more_variables (void)
{
    ...
    v_count += STORE_INCR;
    variables = (bc_var * *)malloc<bc_var *>(v_count*sizeof(bc_var *));
    v_names = (char * *)malloc<char *>(v_count*sizeof(char *));
...
more_arrays (void)
{
    ...
    /* Increment by a fixed amount and allocate. */
    a_count += STORE_INCR;
    arrays = (bc_var_array * *)malloc<bc_var_array *>(a_count*sizeof(bc_var_array *));
    a_names = (char * *)malloc<char *>(a_count*sizeof(char *));
...
void
clear_func (char func)
{
    bc_function * f;
    int indx;
    _Ptr<bc_label_group> lg = ((void *)0);
...
fpush (int val)
{
    ...
    temp = (_TPtr<fstack_rec>)t_malloc<fstack_rec>(sizeof (fstack_rec));
...
pop (void)
{
    _TPtr<Tstruct estack_rec> temp = NULL;
...
void
push_copy (bc_num num : itype(_Ptr<bc_struct>))
{
_TPtr<Tstruct estack_rec> temp = NULL;

temp = (_TPtr<Tstruct estack_rec>)t_malloc<estack_rec>(sizeof (estack_rec));
...
void
push_num (bc_num num : itype(_Ptr<bc_struct>))
{
_TPtr<Tstruct estack_rec> temp = NULL;
...
bc_var *get_var(int var_name) : itype(_Ptr<bc_var>)
        ...
        if (var_ptr == NULL)
        {
            var_ptr = variables[var_name] = (bc_var *)malloc<bc_var>(sizeof (bc_var));
...
bc_num *get_array_num(int var_index, long index) : itype(_Ptr<bc_num>)
        ...
        if (ary_ptr == NULL)
        {
            ary_ptr = arrays[var_index] = (bc_var_array *)malloc<bc_var_array>(sizeof (bc_var_array));
        ...
        if (a_var == NULL) {
            a_var = ary_ptr->a_value = (_Ptr<bc_array>)malloc<bc_array>(sizeof (bc_array));
            a_var->a_tree = NULL;
            a_var->a_depth = 0;
        }
        ...
        {
            temp = (bc_array_node *)malloc<bc_array_node>(sizeof(bc_array_node));
            if (a_var->a_depth != 0)
            {
                temp->n_down[0] = a_var->a_tree;
        ...
        {
            ix1 = sub[log];
            if (temp->n_down[ix1] == NULL)
            {
                temp->n_down[ix1] = (bc_array_node *)malloc<bc_array_node>(sizeof(bc_array_node));
...
auto_var (int name)
{
    ...
    if (name > 0)
    {
        /* A simple variable. */
        ix = name;
        v_temp = (bc_var *)malloc<bc_var>(sizeof (bc_var));
    ...
    else
    {
        /* An array variable. */
        ix = -name;
        a_temp = (bc_var_array *)malloc<bc_var_array>(sizeof (bc_var_array));
...
void
free_a_tree (_Ptr<bc_array_node> root, int depth)
{
    int ix;
    ...
    if (root != NULL)
    {
        if (depth > 1)
            for (ix = 0; ix < NODE_SIZE; ix++)
                free_a_tree (_Assume_bounds_cast<_Ptr<bc_array_node>>(root->n_down[ix]), depth-1);
        else
            for (ix = 0; ix < NODE_SIZE; ix++)
                free_num ( _Assume_bounds_cast<_Ptr<bc_num>>(&root->n_num[ix]));
        free<bc_array_node>(root);
    }
}
...
void
pop_vars (arg_list *list : itype(_Ptr<arg_list>))
{
bc_var * v_temp;
bc_var_array * a_temp;
int    ix;
...
{
free_a_tree (_Assume_bounds_cast<_Ptr<bc_array_node>>(a_temp->a_value->a_tree),
a_temp->a_value->a_depth);
free<bc_array>(a_temp->a_value);
}
free<bc_var_array>(a_temp);
}
}
list = list->next;
}
...
void
process_params (_Ptr<program_counter> pc, int func)
{
    char ch;
    _TPtr<Tstruct arg_list> params= NULL;
    char warned = FALSE;
    int ix, ix1;
    bc_var * v_temp;
    bc_var_array * a_src;
    bc_var_array * a_dest;
    _Ptr<bc_num> n_temp = ((void *)0);
