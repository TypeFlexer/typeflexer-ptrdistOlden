...
static _TPtr<char> t_strcopyof(char *str)
        {
                _TPtr<char>  temp = NULL;
                temp = (_TPtr<char> )t_malloc<char>(strlen (str)+1);
                strcpy (temp,str);
                return (temp);
        }
...
_TPtr<arg_list> nextarg(_TPtr<arg_list> args, char val)
{
    _TPtr<arg_list> temp = NULL;

    temp = (_TPtr<arg_list>)t_malloc<arg_list>(sizeof (arg_list));
    temp->av_name = val;
    temp->next = args;
...
static char* arglist1 = ((void *)0);
static char* arglist2 = ((void *)0);
...
static char *make_arg_str(_TPtr<arg_list> args, int len, int commas) : itype(_Ptr<char>);
...
static char *make_arg_str(_TPtr<arg_list> args, int len, int commas) : itype(_Ptr<char>)
        {
                ...
                /* Recursive call. */
                if (args != NULL)
                temp = make_arg_str (_Tainted_Assume_bounds_cast<_TPtr<arg_list>>(args->next), len+11, commas);
                else
                {
    ...
    if (len != 1 && commas)
        sprintf (((char *)sval), "%d,", args->av_name);
    else
        sprintf (((char *)sval), "%d", args->av_name);
...
char* arg_str(_TPtr<arg_list> args, int commas)
{
    if (arglist2 != NULL)
        free<char>(arglist2);
    arglist2 = arglist1;
...
void
free_args (_TPtr<arg_list> args)
{
    _TPtr<arg_list> temp = ((void *)0);

    temp = args;
    while (temp != NULL)
        _Checked {
        args = args->next;
        t_free<arg_list>(temp);
        temp = args;
    }
}
...
void
check_params (_TPtr<arg_list> params, _TPtr<arg_list> autos)
{
    _TPtr<arg_list>  tmp1 = NULL;
    _TPtr<arg_list>  tmp2 = NULL;
...
int insert_id_rec (_TPtr<id_rec>* root, _TPtr<id_rec> new_id)
{
    _TPtr<id_rec> A = ((void *)0);
    _TPtr<id_rec> B = ((void *)0);
...
/* We need to make a new item. */
        id = (_TPtr<id_rec>)t_malloc<id_rec>(sizeof (id_rec));
        id->id = t_strcopyof (name);
        id->a_name = 0;
        id->f_name = 0;
        id->v_name = 0;
...
yyerror (char* str, ...)
{
    ...
    vfprintf (stderr, str, ((struct __va_list_tag *)args));
    fprintf (stderr, "\n");
    had_error = TRUE;
    va_end (args);
}
...
warn (char* mesg, ...)
{
    ...
    if (std_only)
    {
        if (is_std_in)
            ...
            name = g_argv[optind-1];
        fprintf (stderr,"%s %d: ",name,line_no);
        vfprintf (stderr, mesg, ((struct __va_list_tag *)args));
        fprintf (stderr, "\n");
        had_error = TRUE;
    }
    else
...
rt_error (char* mesg, ...)
{
    ...
    vsprintf (((char *)error_mesg), mesg, ((struct __va_list_tag *)args));
    va_end (args);
