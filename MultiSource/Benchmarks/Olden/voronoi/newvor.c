...
#include <malloc.h>
#endif
_TPtr<Tstruct EDGE_STACK> allocate_stack(int num_vertice);
Tstruct get_point get_points(int n, double curmax,int i, int seed,
                             int processor, int numnodes);

VERTEX_PTR *vp ;
_TPtr<Tstruct VERTEX> va ;
EDGE_PTR *next ;
VERTEX_PTR *org ;
...
_TPtr<void> myalign(int align_size, int alloc_size)
{
    _TPtr<char> base = (_TPtr<char>)t_malloc(alloc_size + align_size);
    _TPtr<void> Result = NULL;
    if (base == NULL){
        printf("myalign() failed\n");
        exit(-1);
    }
#pragma TLIB_SCOPE on
    return (_TPtr<void>)(base + align_size - ((uptrint)base % align_size));
#pragma TLIB_SCOPE off
}
...
QUAD_EDGE alloc_edge() {
  ...
  if (avail_edge == NYL) {
    ans = (QUAD_EDGE)myalign(4*(sizeof(Tstruct edge_rec)),
                             4*(sizeof(Tstruct edge_rec)));
#pragma TLIB_SCOPE on
    if ((uptrint)ans & ANDF) {
#pragma TLIB_SCOPE off
...
int main(int argc, char **argv) {
  _TPtr<Tstruct EDGE_STACK> my_stack = NULL;
  Tstruct get_point point, extra;
  QUAD_EDGE edge;
  int n, retained;
...
_TPtr<Tstruct EDGE_STACK> allocate_stack(int num_vertice)
{
  ...
  my_stack = (_TPtr<Tstruct EDGE_STACK>)t_malloc(sizeof(Tstruct EDGE_STACK));
  my_stack->elts = (_TPtr<QUAD_EDGE>)t_malloc(num_edgeparts * sizeof(QUAD_EDGE));
...
void free_all(cont,my_stack)
     int cont;
     _TPtr<Tstruct EDGE_STACK> my_stack;
{
  t_free(my_stack->elts);
  t_free(my_stack);
}
                            ...
                            int processor, int numnodes)
{
  ...
  node = (VERTEX_PTR) t_malloc(sizeof(Tstruct VERTEX));
...
QUAD_EDGE pop_edge(_TPtr<Tstruct EDGE_STACK> x) {
  int a=x->ptr--;
  return (x)->elts[a];
}

void push_edge(_TPtr<Tstruct EDGE_STACK> stack, QUAD_EDGE edge) {
  register int a;
  /*printf("pushing edge \n");*/
...
void push_ring(_TPtr<Tstruct EDGE_STACK> stack, QUAD_EDGE edge) {
    QUAD_EDGE nex;
    nex = onext(edge);
    while (nex != edge) {
	if (seen(nex) == 0) {
...
void push_nonzero_ring(stack, edge)
     _TPtr<Tstruct EDGE_STACK> stack;
     QUAD_EDGE edge;
{
  QUAD_EDGE nex;
  nex = onext(edge);
