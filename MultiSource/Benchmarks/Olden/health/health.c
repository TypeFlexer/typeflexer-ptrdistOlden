...
_Ptr<struct Village> alloc_tree(int level, int label, _Ptr<struct Village> back) {
  if (level == 0)
    ...
    for (i = 3; i >= 0; i--)
      fval[i] = ((struct Village *)alloc_tree(level - 1, label*4 + i + 1, new));
...
struct Results get_results(_Ptr<struct Village> village) {
  ...
  for (i = 3; i > 0; i--) {
    ...
    fval[i] = get_results(_Assume_bounds_cast<_Ptr<struct Village>>(V));
  }

  fval[0] = get_results(_Assume_bounds_cast<_Ptr<struct Village>>(village->forward[0]));
...
int main(int argc, _Ptr<_Ptr<char>> argv)
{
...
for (i = 3; i > 0; i--) {
struct Village *V = village->forward[i];
struct List *L = ((struct List *)sim(_Assume_bounds_cast<_Ptr<struct Village>>(V)));
val[i] = L;
}

val[0] = ((struct List *)sim(_Assume_bounds_cast<_Ptr<struct Village>>(village->forward[0])));
h = &village->hosp;
